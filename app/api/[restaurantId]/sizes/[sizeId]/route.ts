...
export async function PATCH (req: Request, { params }: { params: {restaurantId: string, sizeId: string}}) {
    try {
        const { userId } = auth();
...
export async function DELETE (req: Request, { params }: { params: {sizeId: string, restaurantId: string}}) {
    try {
        const { userId } = auth();
