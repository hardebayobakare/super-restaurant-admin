...
export const SizeForm: React.FC<SizeFormProps> = ({
    ...
    const onSubmit = async (data: SizeFormValues) => {
        try {
            setLoading(true);
            if (initialData) {
                await axios.patch(`/api/${params.restaurantId}/sizes/${params.sizeId}`, data);
            } else {
                await axios.post(`/api/${params.restaurantId}/sizes`, data);
            }
            router.refresh();
            router.push(`/${params.restaurantId}/sizes`)
    ...
    const onDelete = async  () => {
        try {
           setLoading(true);
           await axios.delete(`/api/${params.restaurantId}/sizes/${params.sizeId}`)
           router.refresh();
           router.push(`/${params.restaurantId}/sizes`);
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input disabled={ loading } placeholder="Size Vlaue" {...field}/>
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} 
